...
const Search = () => {
  ...
  const getRooms = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BASE_URL}/api/room`);
      if (res.status === 200) {
  ...
  const handleSearch = () => {
    if (category === "All") {
      ...
      const filteredRooms = listStudio.filter(
        (room) => room.category === category.toLowerCase()
      );
